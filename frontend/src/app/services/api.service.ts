...
export interface AttendanceSummary { total: number; present: number; absent: number; late?: number; leave?: number; percentage: number; }
...
export class ApiService {
  ...
  getSessions(): Observable<AttendanceSession[]> {
    return this.http.get<AttendanceSession[]>(`${this.baseUrl}/sessions`, { headers: this.authHeaders() });
  }
  ...
  getAttendanceSummary(date: string): Observable<AttendanceSummary> {
    return this.http.get<AttendanceSummary>(`${this.baseUrl}/attendance/summary/${date}`, { headers: this.authHeaders() });
  }
